...
public class MoveSample : MoveBase
{
    ...
    public void SetUpPlayableDirector(PlayableDirector _director, UnitBase _executorUnit, UnitBase _targetUnit)
    {
        void AssignAnimationBinding(UnitBase _Unit, PlayableBinding _playableBinding)
        {
            void ReplaceTrackClips(AnimationTrack _track)
            {
                ...
                foreach (var clip in clips)
                {
                    ...
                    UnitBase.UnitAnimation unitAnimation = Array.Find(_Unit.m_unitAnimations, i => i.m_name == clip.displayName);
    ...
    public static void BindToDirector(string _streamName, UnityEngine.Object _objectToBind)
    {
        ...
        if (!FindObjectOfType<TurnSystemBase>().GetComponent<PlayableDirector>().TryGetPlayableBindingByName(_streamName, out playableBinding)) return;
